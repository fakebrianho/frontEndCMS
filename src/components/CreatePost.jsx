...
export function CreatePost() {
	...
	const handleSubmit = async (values) => {
		try {
			...
			console.log('Creating new post with values:', postData)
			await createPost(postData)

			if (values.prev && !values.isSectionHead) {
				console.log('Updating previous post:', {
					prevPost: values.prev,
					updateData: { next: fileName },
				})
				await updatePost(values.prev, { next: fileName })
			}
			if (values.next && !values.isSectionHead) {
				await updatePost(values.next, { prev: fileName })
			}
			if (values.parent) {
				console.log('Updating parent post:', {
					parentPost: values.parent,
					updateData: { children: fileName },
				})
				await updateParentPost(values.parent, fileName)
			}
