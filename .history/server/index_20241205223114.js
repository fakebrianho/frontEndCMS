...
app.patch('/api/posts/:title', async (req, res) => {
	try {
		console.log('Attempting to update post:', req.params.title)
		console.log('Update data:', req.body)

		// First check if the post exists
		const existingPost = await posts.findOne({ title: req.params.title })
		if (!existingPost) {
			console.log('Post not found:', req.params.title)
			return res.status(404).json({ error: 'Post not found' })
		}
